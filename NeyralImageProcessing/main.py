...
def process_image(input_path, output_path):
    path = "experiments\\images\\21styles\\"
    print()
    path_to_style = path + random.choice(
        list(filter(lambda s: s.endswith(".jpg") or s.endswith(".png") or s.endswith(".jpeg"), os.listdir(path=path))))

    Args = namedtuple('args', ['content_image', 'style_image', 'output_image', 'content_size', 'style_size', 'ngf','model','cuda'])
    args = Args(input_path, path_to_style, output_path, 512, 512, 128,'models\\21styles.model',0)
